...
export interface IOpAttributes {
...
export interface IUrlSanitizerFn {
  (url: string): string | undefined;
}
export interface IOpAttributeSanitizerOptions {
...
export class OpAttributeSanitizer {
  ...
  static IsValidHexColor(colorStr: string) {
    return !!colorStr.match(/^#([0-9A-F]{8}|[0-9A-F]{6}|[0-9A-F]{3})$/i);
  }
  ...
  static IsValidRGBAColor(colorStr: string) {
    const re =
      /^rgba\(((0|25[0-5]|2[0-4]\d|1\d\d|0?\d?\d),\s*){2}(0|25[0-5]|2[0-4]\d|1\d\d|0?\d?\d),\s*(0|1|0?\.\d+)\)$/i;
    return !!colorStr.match(re);
  }
